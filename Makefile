...
#EXTENSION_DIR		=	$(shell /system/php/bin/php-config --extension-dir)
EXTENSION_DIR		= /system/php/lib/
...
DBUSFLAGS            =   `pkg-config --cflags gio-unix-2.0`
DBUSLIBS             =   `pkg-config --libs gio-unix-2.0`



COMPILER_FLAGS		=	-Wall -c -O2 -std=c++11 -fpic `pkg-config --libs --cflags gio-unix-2.0 --libs dbus-1` -o
LINKER_FLAGS		=	-shared ${DBUSFLAGS}
LINKER_DEPENDENCIES	=	-lphpcpp  ${DBUSLIBS}

#COMPILER_FLAGS      +=   -Wall -Wno-inconsistent-missing-override -c -std=c++11 -fpic ${DBUSLIBS} -o
#LINKER_FLAGS        =   -shared ${DBUSFLAGS}
#LINKER_DEPENDENCIES =   -lphpcpp ${DBUSLIBS}
